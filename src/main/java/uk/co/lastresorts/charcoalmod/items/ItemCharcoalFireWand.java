...
public class ItemCharcoalFireWand extends Item implements ICharcoalTool {
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List info, boolean useExtraInformation) {
		...
		info.add("the properties of fire.");
	}
	
	@Override
	public void registerIcons(IIconRegister reg) {
	    for (int i = 0; i < 2; i ++) {
	        this.icons[i] = reg.registerIcon(CharcoalMod.MODID + ":charcoalFireWand_" + i);
	    }
	}
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if(!world.isRemote) {
        	...
        	if(!player.isSneaking()) {
        		world.spawnEntityInWorld(new EntityFireProjectile(world, player, false));
        		stack.damageItem(1, player);
        	}else{
        		world.spawnEntityInWorld(new EntityFireProjectile(world, player, true));
