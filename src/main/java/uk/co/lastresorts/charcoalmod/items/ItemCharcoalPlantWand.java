...
public class ItemCharcoalPlantWand extends Item implements ICharcoalTool {
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List info, boolean useExtraInformation) {
		...
		info.add("the properties of plants.");
	}
	
	@Override
	public void registerIcons(IIconRegister reg) {
	    for (int i = 0; i < 2; i ++) {
	        this.icons[i] = reg.registerIcon(CharcoalMod.MODID + ":charcoalPlantWand_" + i);
	    }
	}
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if(!world.isRemote) {
        	...
        	if(!player.isSneaking() /*&& !player.isSprinting()*/) {
        		world.spawnEntityInWorld(new EntityPlantProjectile(world, player, false, false));
        		stack.damageItem(1, player);
        	/*}else if(player.isSprinting() && !player.isSneaking()){
        		world.spawnEntityInWorld(new EntityPlantProjectile(world, player, false, true));
        		stack.damageItem(3, player);*/
        	}else{
        		world.spawnEntityInWorld(new EntityPlantProjectile(world, player, true, false));
